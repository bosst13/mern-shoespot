...
const StatusTable = () => {
  ...
  useEffect(() => {
    const fetchStatuses = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/v1/orders/statuses');
  ...
  const handleSave = async () => {
    try {
      await axios.put(`http://localhost:5000/api/v1/orders/statuses/${editStatus.orderId}`, editStatus);
